...
This is step by step walkthrough guide to getting started with developing extensions for Visual Studio for Mac 2022 with explanations, code examples, and a couple of links to official documentation

The extensibility story for [Visual Studio for Mac](https://learn.microsoft.com/visualstudio/mac/what-happened-to-vs-for-mac?WT.mc_id=DT-MVP-5004822) was almost non-existent for a while, and the [documentation for getting started](https://learn.microsoft.com/en-us/previous-versions/visualstudio/mac/extending-visual-studio-mac-walkthrough?view=vsmac-2017&WT.mc_id=DT-MVP-5004822) was really outdated. [Visual Studio for Mac](https://learn.microsoft.com/visualstudio/mac/what-happened-to-vs-for-mac?WT.mc_id=DT-MVP-5004822) was originally a re-branding of Xamarin Studio, which was built over MonoDevelop and the extensibility SDK's we used for the longest time was all from the old MonoDevelop Addin libraries. The original [getting started guide from MonoDevelop](https://www.monodevelop.com/developers/articles/creating-a-simple-add-in/) is still somewhat correct, but the libraries referred to in the guide will no longer build.

For [Visual Studio for Mac 2022](https://learn.microsoft.com/visualstudio/mac/what-happened-to-vs-for-mac?WT.mc_id=DT-MVP-5004822) this has changed and now we can create [Visual Studio for Mac](https://learn.microsoft.com/visualstudio/mac/what-happened-to-vs-for-mac?WT.mc_id=DT-MVP-5004822) extensions using the [Microsoft.VisualStudioMac.Sdk](https://www.nuget.org/packages/Microsoft.VisualStudioMac.Sdk) library that we can install from [nuget.org](https://www.nuget.org/packages/Microsoft.VisualStudioMac.Sdk). To make things even better, we can now break free of our old .NET Framework 4.x shackles and start targetting .NET 7.0. and all it's goodness
...
In this walkthrough, we will build a simple [Visual Studio for Mac](https://learn.microsoft.com/visualstudio/mac/what-happened-to-vs-for-mac?WT.mc_id=DT-MVP-5004822) extension that adds the **Insert Text** menu item to the **Edit** menu. All this can do is to insert the text `// Hello` to the active document from the current cursor position

### Step 1 - Create New Project

Let's start with creating a new project called `Sample.csproj`

Here's how a csproj file for an empty [Visual Studio for Mac](https://learn.microsoft.com/visualstudio/mac/what-happened-to-vs-for-mac?WT.mc_id=DT-MVP-5004822) extension project looks like:
...
A [Visual Studio for Mac](https://learn.microsoft.com/visualstudio/mac/what-happened-to-vs-for-mac?WT.mc_id=DT-MVP-5004822) extension has metadata about its name, version, dependencies, etc. It also defines any number of extensions that plug into extension points defined by other extensions, and can also define extension points that other extensions can extend.
...
The combined `Id` and `Namespace` from `Addin` should be **unique among all [Visual Studio for Mac](https://learn.microsoft.com/visualstudio/mac/what-happened-to-vs-for-mac?WT.mc_id=DT-MVP-5004822) extensions**. The other attributes are self-explanatory
...
This can be done by right clicking on the extension project from [Visual Studio for Mac](https://learn.microsoft.com/visualstudio/mac/what-happened-to-vs-for-mac?WT.mc_id=DT-MVP-5004822) then selecting **Pack** from the context menu
...
To create the `.mpack` package from the command line, we need to use the **Visual Studio Tool Runner** a.k.a. `vstool`. The **Visual Studio Tool Runner** is included in the [Visual Studio for Mac](https://learn.microsoft.com/visualstudio/mac/what-happened-to-vs-for-mac?WT.mc_id=DT-MVP-5004822) installation. The **Visual Studio Tool Runner** is available from the following path
...
Debugging a [Visual Studio for Mac](https://learn.microsoft.com/visualstudio/mac/what-happened-to-vs-for-mac?WT.mc_id=DT-MVP-5004822) is possible, but doesn't come out of the box. To enable Debugging the extension from [Visual Studio for Mac](https://learn.microsoft.com/visualstudio/mac/what-happened-to-vs-for-mac?WT.mc_id=DT-MVP-5004822) we need to add the following to our C# project
...
Debugging the extension will basically start another instance of [Visual Studio for Mac](https://learn.microsoft.com/visualstudio/mac/what-happened-to-vs-for-mac?WT.mc_id=DT-MVP-5004822) where you can test your extension
...
If you followed Step 5, then you should already have a `.mpack` at hand. To install a [Visual Studio for Mac](https://learn.microsoft.com/visualstudio/mac/what-happened-to-vs-for-mac?WT.mc_id=DT-MVP-5004822) extension, you need to follow these steps:
...
You need to restart [Visual Studio for Mac](https://learn.microsoft.com/visualstudio/mac/what-happened-to-vs-for-mac?WT.mc_id=DT-MVP-5004822) at this point before you can see our new extension under the **Edit** menu
